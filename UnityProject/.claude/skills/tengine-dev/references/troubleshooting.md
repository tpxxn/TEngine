...
调试器（`~` 键）查看资源引用计数。常见原因：忘记 `UnloadAsset`、静态变量持有引用、UI Widget Asset 未释放。场景切换后调用 `ForceUnloadUnusedAssets(gcCollection: true)`。详见 [resource-api.md](resource-api.md)。
...
3. 实现类已 `GameEventHelper.RegisterListener<IXxx>()` 注册
4. Source Generator 已重新生成
...
public static void SetSprite(this Image image, string location,
    bool setNativeSize = false, Action<Image> callback = null,      // ← Action<Image>
...
// ❌ 错误用法
image.SetSprite("icon", callback: (Sprite s) => { });              // Action<Sprite> 不匹配
// ✅ 正确用法
image.SetSprite("icon", callback: (Image img) => { });             // Action<Image>
```
